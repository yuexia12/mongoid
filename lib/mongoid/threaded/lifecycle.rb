...
module Mongoid #:nodoc:
  module Threaded #:nodoc:
    ...
    module Lifecycle
      ...
      # @since 2.2.0
      def _assigning
        Threaded.begin_stack(:assign)
        yield
      ensure
        Threaded.exit_stack(:assign)
      end
      ...
      # @since 2.1.0
      def _binding
        Threaded.begin_stack(:bind)
        yield
      ensure
        Threaded.exit_stack(:bind)
      end
      ...
      # @since 2.1.0
      def _building
        Threaded.begin_stack(:build)
        yield
      ensure
        Threaded.exit_stack(:build)
      end
      ...
      # @since 2.3.2
      def _loading
        Threaded.begin_stack(:load)
        yield
      ensure
        Threaded.exit_stack(:load)
      end
      ...
      def _loading_revision
        Threaded.begin_stack(:load_revision)
        yield
      ensure
        Threaded.exit_stack(:load_revision)
      end
